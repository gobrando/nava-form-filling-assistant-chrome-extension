...
| Phase 1 — find the form | The page agent inventories visible `input`, `select`, and `textarea` elements. It removes search controls, hidden controls, honeypots, uploads, passwords/OTP/payment fields, and bot-check fields. A page with no candidates is reported, not filled. |
...
| Phase 6 — human submit gate | The page agent can inspect submit control state and bot-token state, but there is no message or code path that clicks submit. The review UI sends the caseworker back to the application. |
...
2. **Page-level control.** Generic auto-navigation is a high-risk source of accidental submit-adjacent clicks. The first prototype fills one visible page and lets the caseworker advance.
...
3. Add a form-navigation policy for the three known sites only. Every allowed Next/Continue control should be named in a versioned playbook; deny all submit-adjacent text.
